// Package components defines the protocol-layer decomposition of caphouse's
// columnar storage model.
//
// # Overview
//
// Caphouse parses each frame into its constituent protocol layers and stores
// each layer in its own ClickHouse table. This makes packet data queryable at
// the column level while still allowing lossless PCAP reconstruction via
// [ReconstructFrame].
//
...
// decode order. The bitmask in PacketNucleus.Components records which kinds are
// present for a given packet. Use [ComponentHas] to test the mask.
//
// Some orders are repeatable (e.g. OrderL2Tag for 802.1Q stacking, OrderL3Ext
// for IPv6 extension headers); see [OrderRepeatable].
//
// # Registries
//
// [ComponentFactories] maps each kind constant to a zero-value constructor.  It
// is the authoritative list of all registered components and is used throughout
// caphouse to build schema, drive batch inserts, and generate SQL.
//
...
// Embed CaptureID, PacketID, and CodecVersion as the first three fields — these
// are required by the schema conventions:
//
// type MyComponent struct { CaptureID    uuid.UUID `ch:"capture_id"` PacketID
// uint64    `ch:"packet_id"` CodecVersion uint16    `ch:"codec_version"` // ...
// protocol fields }
//
...
//
// func (c *MyComponent) ClickhouseColumns() ([]string, error) { return
// GetClickhouseColumnsFrom(c) } func (c *MyComponent) ClickhouseValues()
// ([]any, error) { return GetClickhouseValuesFrom(c) }
//
// 3. Write the ClickHouse schema as an embedded SQL file (e.g. my_schema.sql)
// and expose it via Schema():
//
// //go:embed my_schema.sql var mySchemaSQL string
//
// func (c *MyComponent) Schema(table string) string { return
// applySchema(mySchemaSQL, table) }
//
...
//
// // In types.go, add to ComponentFactories: ComponentMyProto: func() Component
// { return &MyComponent{} },
//
